...
export const SavedScreen: React.FC<{ navigate: (to: string) => void }> = ({ navigate }) => {
	const { devices, unsyncDevice, setEditingDeviceId } = useBluetooth();
			...
			<FlatList
				...
				renderItem={({ item, index }) => (
					<View style={[styles.itemContainer, index === 0 && styles.firstItem, index === devices.length - 1 && styles.lastItem]}>
						<ConnectionItem device={item} onEdit={(d) => { setEditingDeviceId(d.id); navigate('edit'); }} onForget={(id) => unsyncDevice(id)} />
					</View>
				)}
			/>
		</View>
